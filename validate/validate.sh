...
# get cleos path
read -p "Please input the local cleos executable path, such as ../../ogetc/build/programs/cleos/cleos : " cleospath
...
# system accounts
SYS_ACCOUNTS=( "ogetcio.bpay" "ogetcio.msig" "ogetcio.names" "ogetcio.ram" "ogetcio.ramfee" "ogetcio.saving" "ogetcio.stake" "ogetcio.token" "ogetcio.vpay" "ogetcio.gns" "ogetcio.gstake" "ogetcio.vs" )

# fund accounts
FUND_ACCOUNTS=( "ogetc.don" "ogetc.vol" "ogetc.exp" "ogetc.eco" )
...
act_blc=(   [ogetc.don]="0"
            [ogetc.vol]="0"
            [ogetc.exp]="3437010000.0000"
            [ogetc.eco]="4000000000.0000"
...
total_staked=0
all_staked=$($CLEOS get table ogetcio ogetcio voters -l 100 | grep staked | cut -d ":" -f 2 | cut -d "," -f 1)
...
if [ $pass1 == "false" ]
    then
        echo -e "\033[31m*************** The balance validation did not pass, please send this result to ogetclab *************** \033[0m"
    else
...
supposed_owner="1:1ogetcio@active"
supposed_active="1:1ogetcio@active"
ogetcio_supposed_owner="1:1ogetcio.prods@active"
ogetcio_supposed_active="1:1ogetcio.prods@active"
pass2="true"
...
ogetcio_account_owner=$($CLEOS get account ogetcio | grep " owner " | awk '{print $2 $3 $4}' )
ogetcio_account_active=$($CLEOS get account ogetcio | grep " active " | awk '{print $2 $3 $4}' )
if [[ $ogetcio_account_owner == $ogetcio_supposed_owner && $ogetcio_account_active == $ogetcio_supposed_active ]]
    then
        echo "ogetcio authority correct : [$ogetcio_account_owner, $ogetcio_account_active]"
    else
        printf %.s* {1..100} && echo
        echo -e "\033[31m Warning!!!!!!ogetcio supposed authority: [$ogetcio_supposed_owner, $ogetcio_supposed_active], onchain : [$ogetcio_account_owner, $ogetcio_account_active] \033[0m"
        printf %.s* {1..100} && echo
        pass2="false"
fi

# check privilege
all_accounts=( ${SYS_ACCOUNTS[*]} ${FUND_ACCOUNTS[*]} ${ALL_ACCOUNTS[*]} "ogetcio" )
for account in ${all_accounts[*]}
    do
        privilege=$($CLEOS get account ${account} | grep privileged)
        if [[ $account == "ogetcio" || $account == "ogetcio.msig" ]]
            then
                if [ -z "$(echo $privilege)" ]
                    then
                        printf %.s* {1..100} && echo
...
if [ $pass2 == "false" ]
    then
        echo -e "\033[31m************* The authority validation did not pass, please send this result to ogetclab ************* \033[0m"
    else
...
supposed_hash=(     [ogetcio]="33f3b7c17f84e88aca528ebdb37bfa072fc086543298016c2e5db14beec30530"
                    [ogetcio.token]="e87d28ac3b68ad6f84b45be23799eb767c747f0e527078581febc30e2f434531"
                    [ogetcio.msig]="abe3fd766cd42ed25f6aade735118c18b46f59c3198c57a4f4ae04ec58ac0686"  )
pass3="true"
...
if [ $pass3 == "false" ]
    then
        echo -e "\033[31m************* The code hash validation did not pass, please send this result to ogetclab ************* \033[0m"
    else
...
if [[ $pass1 == "true" && $pass2 == "true" && $pass3 == "true" ]]
    then
        ...
        echo -e "******************* Congratulations!!! All the validation passed, let's go ogetc! ********************"
        printf %.s* {1..100} && echo -e "\033[0m\n"
    else
        echo -e "\n\033[31m" && printf %.s* {1..100} && echo
        echo -e "*************** Warning!!!!! Not all tests passed, please send this result to ogetclab ***************"
        printf %.s* {1..100} && echo -e "\033[0m\n"
fi
