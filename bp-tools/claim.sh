...
BIN_DIR='<your programs dir>'      # such as BIN_DIR='/home/ogetclab/ogetc/build/programs'
NODE_HOST='<your nodeos --http-server-address>' # such as NODE_HOST='127.0.0.1:8888' or NODE_HOST='api.ogetclab.io:8080'
...
last_claim_time=`$CLEOS get table ogetcio ogetcio producers -l 1000 | jq -r '.rows[] | select(.owner == "'$BPNAME'") | .last_claim_time'`
...
$CLEOS push action ogetcio claimrewards "{\"owner\":\"$BPNAME\"}" -p $BPNAME@$PERMISSION
if [ $? -eq 0 ]; then
